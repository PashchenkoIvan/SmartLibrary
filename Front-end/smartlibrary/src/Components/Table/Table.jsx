...
const Table = props => {
	...
	const keysMap = keys.map(k => {
		...
		if (k.isCounter === true) {
			return (
				<div style={style}>
					{k.name} ({props.data.length})
				</div>
			);
		} else {
			return <div style={style}>{k.name}</div>;
		}
	});
	...
	const dataMap = props.data.map(el => {
		const propMap = el => {
			return keys.map(k => {
				...
				if (k.isLink === true) {
									...
									if (b.isLink) {
										return (
											<Link
												...
												onClick={() => window.scrollTo(0, 0)}
											>
												<button style={style} className={s.btn}>
													{b.name}
												</button>
											</Link>
										);
					...
					return <div style={style}>{el[k.property] ? '+' : '—'}</div>;
				} else if (k.isPopup) {
					return (
						<div style={style}>
							<Popup
		...
		return <div className={s.tableEl}>{propMap(el)}</div>;
	});

	return (
		<div className={s.container}>
			<div className={s.keysBar}>{keysMap}</div>
			<div className={s.table}>{dataMap}</div>
		</div>
	);
};
